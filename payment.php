...
while ($row = $result->fetch_assoc()) {
    ...
    $properties[$pid]['units'][] = $row;
    $properties[$pid]['property_total'] += $row['unit_total'];
}
$stmt->close();
...
foreach ($properties as $prop) {
    $total_commission += $prop['property_total'] * 0.00002; // ✅ correct 2% rate
}

// ✅ Round to nearest 10 for M-Pesa
$total_commission = round($total_commission / 10) * 10;
...
<body>
    ...
    <main class="container mt-4">
        ...
        <?php if ($stkMessageText): ?>
            <div class="alert alert-<?= htmlspecialchars($stkMessageClass) ?>"><?= htmlspecialchars($stkMessageText) ?></div>
        <?php endif; ?>

        <?php if (empty($properties)): ?>
            <div class="alert alert-warning text-center shadow-sm">No Properties or Units Found.</div>
        <?php else: ?>
            ...
            <div class="text-center payment-buttons mt-4">
                <!-- Paystack -->
                <button type="button" class="btn btn-primary btn-lg" onclick="payWithPaystack()">Pay with Paystack</button>

                <!-- M-Pesa -->
                <form method="POST" action="../Shared/mpesaStk" class="d-inline-block ms-2">
                    <input type="hidden" name="amount" value="<?= htmlspecialchars($total_commission) ?>">
                    <input type="hidden" name="property_id" value="<?= htmlspecialchars(array_key_first($properties)) ?>">
                    <input type="text" name="phone" id="mpesaPhone" class="form-control phone-input d-inline-block mb-2 mb-sm-0"
                        placeholder="Enter M-Pesa phone (07XXXXXXXX)" required>
                    <button type="submit" class="btn btn-success btn-lg">Pay with M-Pesa</button>
                </form>
            </div>
        <?php endif; ?>
    </main>
    ...
    <script>
        function payWithPaystack() {
            var handler = PaystackPop.setup({
                ...
                email: '<?= addslashes($landlord_email) ?>',
